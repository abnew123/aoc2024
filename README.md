...
On a 2024 MacBook Pro running macOS 15.6 (build 24G84), OpenJDK 23.0.1, `aarch64`, and 14 processors, the latest clean current-source 10-process means are 205.186 ms wall time and 145.112 ms summed solver time. The clean counterbalanced publication-gate comparison against the preceding pushed tip measured solver time improving from 158.720 ms to 145.112 ms; the paired 95% confidence interval for that solver delta is [-20.301, -6.915] ms.
...
Day 19 now builds its five-color towel trie once and derives reachability plus exact arrangement counts in one reverse dynamic-programming pass per design, promoting to `BigInteger` only when needed. A clean recovery run after removal of leaked background JVMs measured its isolated mean falling from 12.070 ms to 6.868 ms (-43.1%), with a paired 95% confidence interval of [-5.431, -4.974] ms; the clean whole-suite paired interval also excluded zero.

Day 1 now parses the two lists once, sorts exact arbitrary-size integers once, and obtains similarity through a run-count merge instead of a boxed frequency map. Its isolated mean fell from 10.450 ms to 9.214 ms (-11.8%), with a paired 95% confidence interval of [-1.527, -0.945] ms.
...
The latest batch adds reverse-time union-find connectivity for Day 18 and a turn-to-turn functional graph for Day 6. Each strategy rewrite passed its own counterbalanced isolated 95% confidence gate before the two-change batch passed the full-suite publication gate above. Day 6's isolated mean fell from 16.873 ms to 6.702 ms (-60.3%), with a paired 95% confidence interval of [-10.378, -9.964] ms.

Day 11 now shares one exact blink memo across both parts, and Day 21 replaces complete command-string enumeration plus hardcoded rewrites with weighted transition dynamic programming over the two keypads. Day 21's isolated mean fell from 14.946 ms to 3.724 ms (-75.1%), with a paired 95% confidence interval of [-11.624, -10.822] ms.
